...
async def get_price_by_symbol(symbol: str):
    ...
    if symbol not in prices:
        raise NotFound(detail=f"Currency '{symbol}' not found.")
...
async def get_gold_by_symbol(symbol: str):
    ...
    if symbol not in prices:
        raise NotFound(detail=f"Gold symbol '{symbol}' not found.")
...
async def get_crypto_by_symbol(symbol: str):
    ...
    if symbol not in prices:
        raise NotFound(detail=f"Crypto symbol '{symbol}' not found.")
